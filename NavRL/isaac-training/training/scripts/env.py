...
class NavigationEnv(IsaacEnv):
    ...
    def __init__(self, cfg):
        ...
        # LiDAR params:
        self.lidar_range = cfg.sensor.lidar_range          # LiDAR 最大探测距离
        self.lidar_vfov = (max(-89., cfg.sensor.lidar_vfov[0]), min(89., cfg.sensor.lidar_vfov[1]))  # 垂直视场角（限制在 -89~89 度）
        self.lidar_vbeams = cfg.sensor.lidar_vbeams        # 垂直方向激光束数量
        self.lidar_hres = cfg.sensor.lidar_hres            # 水平分辨率（度/束）
        self.lidar_hbeams = int(360/self.lidar_hres)       # 水平方向总束数（360度 / 分辨率）
        ...
        ray_caster_cfg = RayCasterCfg(
            prim_path="/World/envs/env_.*/Hummingbird_0/base_link",    # 传感器挂载在无人机 base_link 上
            offset=RayCasterCfg.OffsetCfg(pos=(0.0, 0.0, 0.0)),    # 无偏移
            attach_yaw_only=True,     # 只随无人机 yaw 旋转（不随 pitch/roll）
            # attach_yaw_only=False,
            pattern_cfg=patterns.BpearlPatternCfg(
                horizontal_res=self.lidar_hres, # horizontal default is set to 10   
                vertical_ray_angles=torch.linspace(*self.lidar_vfov, self.lidar_vbeams)   # 均匀分布垂直角度
            ),
            debug_vis=False,   # 不显示调试射线
            mesh_prim_paths=["/World/ground"],   # 只检测地面（可改为 ["/World"] 检测所有物体）
            # mesh_prim_paths=["/World"],
        )
        self.lidar = RayCaster(ray_caster_cfg)
        self.lidar._initialize_impl()
        self.lidar_resolution = (self.lidar_hbeams, self.lidar_vbeams)    # LiDAR 输出分辨率
    ...
    def _design_scene(self):
        # Initialize a drone in prim /World/envs/envs_0  初始化无人机模型
    ...
    def _set_specs(self):
        ...
        stats_spec = CompositeSpec({
            ...
            "episode_len": UnboundedContinuousTensorSpec(1),  # 当前 episode 步数
            "reach_goal": UnboundedContinuousTensorSpec(1),   # 是否到达目标（0/1）
            "collision": UnboundedContinuousTensorSpec(1),    # 是否发生碰撞（0/1）
            "truncated": UnboundedContinuousTensorSpec(1), # 是否因超时结束
        }).expand(self.num_envs).to(self.device)

        # 辅助信息（Info Spec）  完整无人机状态（13 维：位置3 + 速度3 + 姿态4（四元数）+ 角速度3）  
